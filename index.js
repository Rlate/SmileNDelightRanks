...
let listener = app.listen(process.env.PORT, () => {
     console.log('Your bot is currently listening on port: ' + listener.address().port);
});
...
async function onShout(){
  let shoutchannel = await.channels.cache.get(config.shoutchannelid);
  if(firstshout == true){
    firstshout = false;
    ...
    if(currentshout.body){
      shoutchannel.send({embed: {
        ...
        author: {
          ...
          icon_url: `https://www.roblox.com/Thumbs/Avatar.ashx?x=150&y=150&format=png&username=${shout.poster.username}`
        }
      }});
    } else {
      shoutchannel.send({embed: {
...
if(config.shoutchannel.id !== 'false'){
  onShout();
}

fs.readdir('./commands/', async (err, files) => {
    if(err){
        return console.log(chalk.red('An error has occured when checking the commands folder for commands to load: ' + err));
    }
...
client.on(`ready`, async () => {
  console.log(chalk.yellow(figlet.textSync(`rbot`, { horizontalLayout: `full` })));
